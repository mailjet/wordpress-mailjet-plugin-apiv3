...
class WP_Mailjet
{
	...
	public function show_campaigns_menu()
	{
		...
		echo'</h2></div>';
		echo '<iframe width="980px" height="1200" src="https://'.(($this->api->version == '0.1')?'www':(($this->api->version == 'REST')?'app':'www')).'.mailjet.com/campaigns?t='.$this->_get_auth_token().'&show_menu=none&u=WordPress-3.1"></iframe>';
	}
	...
	public function show_stats_menu()
	{
		...
		echo'</h2></div>';
		echo '<iframe width="980px" height="1200" src="https://'.(($this->api->version == '0.1')?'www':(($this->api->version == 'REST')?'app':'www')).'.mailjet.com/stats?t='.$this->_get_auth_token().'&show_menu=none&u=WordPress-3.1"></iframe>';		
	}
	...
	public function show_contacts_menu()
	{
		...
		echo'</h2></div>';
		echo '<iframe width="980px" height="1200" src="https://'.(($this->api->version == '0.1')?'www':(($this->api->version == 'REST')?'app':'www')).'.mailjet.com/contacts/lists?t='.$this->_get_auth_token().'&show_menu=none&u=WordPress-3.1"></iframe>';		
	}
}