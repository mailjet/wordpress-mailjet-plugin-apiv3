    ...
    public function mailjet_enable_sending_cb( $args ) {
        ...
        <fieldset class="settingsSendingFldset">
            <label class="checkboxLabel" for="mailjet_enabled">
                <input name="mailjet_enabled" type="checkbox" id="mailjet_enabled" value="1" 
                <?php
            ...
            <div id="enable_mj_emails" class="sending_options_div 
            <?php
            echo esc_attr($mailjetEnabled) ? 'mj-show' : 'mj-hide';
            ?>
            ">
                <div>
                    <label class="mj-label" for="mailjet_from_name"><b>
                    <?php
...
> 
                ...
                ?>
                <div class="smtpFld">
                    <label class="mj-label" for="mailjet_port"><b>
                    <?php
                ...
                <div class="sslFld">
                    <label class="checkboxLabel" for="mailjet_ssl">
                        <input name="mailjet_ssl"  type="checkbox" id="mailjet_ssl" value="ssl" 
                        <?php
                ...
                <div id="testEmail">
                    <button type="button" id="mailjet_test" class="sendTestEmailBtn mj-toggleBtn" data-target="test_email_collapsible">
                    <?php
                    _e('Send a test', 'mailjet-for-wordpress');
                    ?>
                    </button>
                    <div id="test_email_collapsible" class="mj-hide test_email_collapsible">
                        <label class="mj-label" for="mailjet_test_address"><b>
                        <?php
    ...
    public function mailjet_sending_settings_page_html() {
        ...
        add_settings_section('mailjet_enable_sending_settings', null, array( $this, 'mailjet_section_enable_sending_cb' ), 'mailjet_sending_settings_page');
        // register a new field in the "mailjet_section_developers" section, inside the "mailjet" page
        add_settings_field(
            'mailjet_enable_sending',
        ...
        if (isset($_GET['settings-updated'])) {
            ...
            } elseif ( ! empty($send_test_email_btn) && ! empty($mailjet_test_address)) {
                    // Send a test email
            ...
            if (\true !== $testSent && \false === $executionError) {
                    ...
                    <div class="right">
                        <div class="centered"  style="width:650px;">
                            ...
                            <form action="options.php" method="post">
                                ...
                                ?>
                                <button type="submit" id="enableSendingSubmit" class="mj-btn btnPrimary MailjetSubmit" name="submit">
                                <?php
    ...
    private function checkConnection() {
        ...
        if (Mailjet::getOption('mailjet_ssl') && Mailjet::getOption('mailjet_port') == 465) {
            $encryption = 'ssl';
            $protocol   = 'ssl://';
        } elseif (Mailjet::getOption('mailjet_ssl')) {
                $protocol   = 'tls://';
                $encryption = 'tls';
        }
        if ($encryption == 'ssl' || $encryption == '') {
            $soc = @\fsockopen($protocol . MailjetMail::MJ_HOST, Mailjet::getOption('mailjet_port'), $errno, $errstr, 5);
                ...
                @\stream_socket_enable_crypto($soc, \true, \STREAM_CRYPTO_METHOD_TLS_CLIENT);
        }
        if ($soc) {
            $connected = \TRUE;
