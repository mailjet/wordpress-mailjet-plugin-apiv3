...
class WooCommerceSettings
{
    ...
    public function mailjet_show_extra_woo_fields($checkout)
    {
        ...
        $checkoutBox = Mailjet::getOption('mailjet_woo_checkout_checkbox');
        ...
        if ($contactList !== \false && $checkoutBox === '1') {
            ...
            if (!$contactAlreadySubscribedToList) {
                ...
                $boxMsg = \stripslashes(Mailjet::getOption('mailjet_woo_checkout_box_text')) ?: __('Subscribe to our newsletter', 'mailjet-for-wordpress');
                woocommerce_form_field('mailjet_woo_subscribe_ok', array('type' => 'checkbox', 'label' => __($boxMsg, 'mailjet-for-wordpress'), 'required' => \false), $checkout->get_value('mailjet_woo_subscribe_ok'));
            }
        }
    }
    ...
    {
        if (Mailjet::getOption('mailjet_woo_register_checkbox')) {
            $boxMsg = \stripslashes(Mailjet::getOption('mailjet_woo_register_box_text')) ?: __('Subscribe to our newsletter', 'mailjet-for-wordpress');
    ...
    public function mailjet_subscribe_unsub_woo_to_list($subscribe, $user_email, $first_name, $last_name)
    {
        ...
        if ($action === 'addforce' && get_user_by('email', $user_email) === \false) {
            // check for guest subscription
            $activate_mailjet_woo_integration = Mailjet::getOption('activate_mailjet_woo_integration');
            $mailjet_woo_edata_sync = Mailjet::getOption('mailjet_woo_edata_sync');
    ...
    public function mailjet_subscribe_confirmation_from_woo_form($subscribe, $user_email, $first_name, $last_name)
    {
        ...
        $res = wp_mail($user_email, $email_subject, $message, array('From: ' . Mailjet::getOption('blogname') . ' <' . Mailjet::getOption('admin_email') . '>'));
        return $res;
    }
    ...
    {
        $wooActiv = Mailjet::getOption('activate_mailjet_woo_integration');
        if (!$wooActiv) {
            return \false;
        }
        $mainList = Mailjet::getOption('mailjet_sync_list');
        if ((int) $mainList > 0) {
            return $mainList;
        }
        return \false;
    ...
    {
        $templateId = Mailjet::getOption($templateType);
    ...
    public function toggleWooSettings($activeHooks)
    {
        ...
        $defaultSettings = ['enabled' => 'yes', 'subject' => '', 'heading' => '', 'email_type' => 'html'];
        foreach ($availableActions as $key => $hook) {
            $wooSettings = Mailjet::getOption($hook);
    ...
    private function getTemplateContent($callable)
    {
        ...
        $fromEmail = Mailjet::getOption('mailjet_from_email');
    ...
    public function activateWoocommerce($data)
    {
        ...
        $checkboxesNames = array('activate_mailjet_woo_integration', 'mailjet_woo_edata_sync', 'mailjet_woo_checkout_checkbox', 'mailjet_woo_register_checkbox', 'mailjet_woo_banner_checkbox');
        ...
        foreach ($data as $dataName => $dataValue) {
            if (!\in_array($dataName, $checkboxesNames, \true)) {
        ...
        if ($activate) {
            ...
            add_option('mailjet_woo_abandoned_cart_sending_time', 1200);
            // 20 * 60 = 1200s
    ...
    public function createTemplates($forAbandonedCart = \true, $forOrderNotif = \true)
    {
        $templatesDetails = [];
        if ($forAbandonedCart) {
            $templates['woocommerce_abandoned_cart'] = ['id' => Mailjet::getOption('mailjet_woocommerce_abandoned_cart'), 'callable' => 'abandonedCartTemplateContent'];
        }
        if ($forOrderNotif) {
            $templates['woocommerce_order_confirmation'] = ['id' => Mailjet::getOption('mailjet_woocommerce_order_confirmation'), 'callable' => 'orderCreatedTemplateContent'];
            $templates['woocommerce_refund_confirmation'] = ['id' => Mailjet::getOption('mailjet_woocommerce_refund_confirmation'), 'callable' => 'orderRefundTemplateContent'];
            $templates['woocommerce_shipping_confirmation'] = ['id' => Mailjet::getOption('mailjet_woocommerce_shipping_confirmation'), 'callable' => 'shippingConfirmationTemplateContent'];
        }
        $templateArgs = ["Author" => "Mailjet WC integration", "Categories" => ['e-commerce'], "Copyright" => "Mailjet", "Description" => "Used to send automation emails.", "EditMode" => 1, "IsStarred" => \false, "IsTextPartGenerationEnabled" => \true, "Locale" => "en_US", "Name" => "", "OwnerType" => "apikey", "Presets" => "string", "Purposes" => ['transactional']];
    ...
    public function send_order_status_refunded($orderId)
    {
        ...
        $vars = ['first_name' => $order->get_billing_first_name(), 'order_number' => $orderId, 'order_total' => wc_price($order->get_total()), 'store_email' => Mailjet::getOption('mailjet_from_email'), 'store_name' => get_bloginfo(), 'store_address' => Mailjet::getOption('woocommerce_store_address'), 'order_link' => $order->get_view_order_url()];
        $templateId = Mailjet::getOption('mailjet_woocommerce_refund_confirmation');
    ...
    {
        $order = wc_get_order($orderId);
        $templateId = Mailjet::getOption('mailjet_woocommerce_order_confirmation');
    ...
    {
        $templateId = Mailjet::getOption('mailjet_woocommerce_abandoned_cart');
        if (!$cart || empty($cart) || !$templateId || empty($templateId)) {
            return \false;
        }
        $cartProducts = \json_decode($cart->abandoned_cart_info, \true);
        ...
        $vars = ['store_name' => get_bloginfo(), 'store_address' => Mailjet::getOption('woocommerce_store_address'), 'abandoned_cart_link' => $abandoned_cart_link, 'products' => $products];
    ...
    {
        $order = wc_get_order($orderId);
        $templateId = Mailjet::getOption('mailjet_woocommerce_shipping_confirmation');
        if (!$order || empty($order) || !$templateId || empty($templateId)) {
            return \false;
        }
        ...
        $vars = [
            ...
            'tracking_number' => !empty($tracking_number) ? $tracking_number : 'NA',
            // Arbitrary default value needed in the email template
            'order_total' => wc_price($order->get_total()),
            'order_link' => $order->get_view_order_url(),
            'tracking_url' => $order->get_meta('_wcst_order_track_http_url'),
            'store_email' => Mailjet::getOption('mailjet_from_email'),
            'store_name' => get_bloginfo(),
            'store_address' => Mailjet::getOption('woocommerce_store_address'),
        ];
        $data = $this->getFormattedEmailData($this->getOrderRecipients($order, $vars), $templateId);
        $response = MailjetApi::sendEmail($data);
        if ($response === \false) {
    ...
    private function toggleAbandonedCart()
    {
        ...
        if (Mailjet::getOption('mailjet_woo_abandoned_cart_activate') === '1') {
            $this->createTemplates(\true, \false);
            if (!wp_next_scheduled('abandoned_cart_cron_hook')) {
                wp_schedule_event(\time(), 'one_minute', 'abandoned_cart_cron_hook');
            }
            $this->createAbandonedCartTables();
            $activeHooks = [['hook' => 'woocommerce_add_to_cart', 'callable' => 'cart_change_timestamp'], ['hook' => 'woocommerce_cart_item_removed', 'callable' => 'cart_change_timestamp'], ['hook' => 'woocommerce_cart_item_restored', 'callable' => 'cart_change_timestamp'], ['hook' => 'woocommerce_after_cart_item_quantity_update', 'callable' => 'cart_change_timestamp'], ['hook' => 'woocommerce_calculate_totals', 'callable' => 'cart_change_timestamp'], ['hook' => 'woocommerce_cart_is_empty', 'callable' => 'cart_change_timestamp'], ['hook' => 'woocommerce_order_status_changed', 'callable' => 'update_status_on_order'], ['hook' => 'woocommerce_cheque_process_payment_order_status', 'callable' => 'update_status_paid_by_cheque'], ['hook' => 'abandoned_cart_cron_hook', 'callable' => 'send_abandoned_cart_emails'], ['hook' => 'wp_ajax_nopriv_save_guest_data', 'callable' => 'save_guest_data']];
        } else {
            global $wpdb;
    ...
    private function getAbandonedCartRecipients($cart, $vars)
    {
        ...
        if (isset($email) && is_email($email)) {
            $recipients = ['Email' => $email, 'Name' => $name, 'Vars' => $vars];
        }
        return $recipients;
    }
    ...
    {
        $senderName = Mailjet::getOption('woocommerce_email_from_name');
        $senderEmail = Mailjet::getOption('woocommerce_email_from_email');
    ...
    private function addThankYouSubscription($order)
    {
        ...
        wp_register_script('mailjet-woo-ajax-subscribe', $scriptPath, array('jquery'), \false, \true);
        ...
        $text = \stripslashes(Mailjet::getOption('mailjet_woo_banner_text'));
        $label = \stripslashes(Mailjet::getOption('mailjet_woo_banner_label'));
    ...
    public function subscribeViaAjax()
    {
        ...
        if (isset($post['orderId'])) {
            ...
            wp_send_json_success(['message' => $message, 'success' => $success]);
        } else {
            wp_send_json_error();
        }
    }
    ...
    private function ajaxSubscription($email, $fName, $lName)
    {
        ...
        if (MailjetApi::checkContactSubscribedToList($email, $listId)) {
            return ['success' => \true, 'message' => 'You are already subscribed.'];
        }
        if ($this->mailjet_subscribe_confirmation_from_woo_form(1, $email, $fName, $lName)) {
            $message = __('We have sent the newsletter subscription confirmation link to you: ', 'mailjet-for-wordpress') . $email . '. ' . __('To confirm your subscription you have to click on the provided link.', 'mailjet-for-wordpress');
            return ['success' => \true, 'message' => $message];
        }
    ...
    public function save_guest_data()
    {
        ...
        if (!is_user_logged_in() && is_email($email_address)) {
            $name = \trim($session->get('billing_first_name') . ' ' . $session->get('billing_last_name'));
        ...
        if ($result && $result[0]->security_key === $key) {
            ...
            if ($result[0]->user_type === 'REGISTERED') {
                ...
                if (WC()->cart->get_cart_contents_count() == 0) {
                    $cartInfo = \json_decode($result[0]->abandoned_cart_info, \true);
    ...
    public function init_edata()
    {
        ...
        $propertyTypes = [SubscriptionOptionsSettings::PROP_USER_FIRSTNAME => 'str', SubscriptionOptionsSettings::PROP_USER_LASTNAME => 'str', SubscriptionOptionsSettings::WP_PROP_USER_ROLE => 'str', self::WOO_PROP_TOTAL_ORDERS => 'int', self::WOO_PROP_TOTAL_SPENT => 'float', self::WOO_PROP_LAST_ORDER_DATE => 'datetime', self::WOO_PROP_ACCOUNT_CREATION_DATE => 'datetime'];
        ...
        $newSegments = array('Newcomers' => array('expr' => '(IsInPreviousDays(' . self::WOO_PROP_ACCOUNT_CREATION_DATE . ',30))', 'description' => __('Customers who have created an account in the past 30 days', 'mailjet-for-wordpress')), 'Potential customers' => array('expr' => '(' . self::WOO_PROP_TOTAL_ORDERS . '<1)', 'description' => __('Contacts that don\'t have any orders', 'mailjet-for-wordpress')), 'First time customers' => array('expr' => '(' . self::WOO_PROP_TOTAL_ORDERS . '=1) and (IsInPreviousDays(' . self::WOO_PROP_LAST_ORDER_DATE . ',30))', 'description' => __('Customers who have made their first purchase in the past 30 days', 'mailjet-for-wordpress')), 'Recent customers' => array('expr' => '(IsInPreviousDays(' . self::WOO_PROP_LAST_ORDER_DATE . ',30))', 'description' => __('Customers who have purchased in the past 30 days', 'mailjet-for-wordpress')), 'Repeat customers' => array('expr' => '(' . self::WOO_PROP_TOTAL_ORDERS . '>1)', 'description' => __('Customers who have purchased more than once', 'mailjet-for-wordpress')), 'Lapsed customers' => array('expr' => '(not IsInPreviousDays(' . self::WOO_PROP_LAST_ORDER_DATE . ',180))', 'description' => __('Customers who haven\'t purchased in the past 6 months', 'mailjet-for-wordpress')));
    ...
    public function all_customers_edata_sync()
    {
        ...
        foreach ($subscribers as $sub) {
            ...
            if (\is_array($properties) && !empty($properties)) {
                $subscribedContacts[] = array('Email' => $email, 'Properties' => $properties);
            }
        }
        foreach ($unsubUsers as $user) {
            ...
            if (!empty($userEmail)) {
                ...
                if (\is_array($properties) && !empty($properties)) {
                    $unsubContacts[] = array('Email' => $user->user_email, 'Properties' => $properties);
                }
            }
        }
        foreach ($unsubGuestProperties as $guestEmail => $guestProperties) {
            if (\is_array($guestProperties) && !empty($guestProperties)) {
                $unsubContacts[] = array('Email' => $guestEmail, 'Properties' => $guestProperties);
            }
        }
        $success = \true;
    ...
    private function merge_customer_and_guest_edata($userData, $guestData)
    {
        ...
        if (isset($userData[self::WOO_PROP_LAST_ORDER_DATE], $guestData[self::WOO_PROP_LAST_ORDER_DATE]) && $guestData[self::WOO_PROP_LAST_ORDER_DATE] > $userData[self::WOO_PROP_LAST_ORDER_DATE]) {
            $mergedData[self::WOO_PROP_LAST_ORDER_DATE] = $guestData[self::WOO_PROP_LAST_ORDER_DATE];
        } else {
            if (!isset($userData[self::WOO_PROP_LAST_ORDER_DATE]) && isset($guestData[self::WOO_PROP_LAST_ORDER_DATE])) {
                $mergedData[self::WOO_PROP_LAST_ORDER_DATE] = $guestData[self::WOO_PROP_LAST_ORDER_DATE];
            }
        }
        return $mergedData;
    ...
    {
        $args = array('customer_id' => 0, 'orderby' => 'date', 'order' => 'ASC', 'type' => 'shop_order', 'limit' => -1);
        if (!empty($guestEmail)) {
