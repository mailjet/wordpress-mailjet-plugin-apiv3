...
<fieldset class="settingsSubscrFldset">
    <label class="mailjet-label" for="activate_mailjet_sync">
    <?php
    _e('Automatically add WordPress users to a Mailjet list', 'mailjet-for-wordpress');
    ?>
    </label>
    <label class="checkboxLabel">
        <input name="activate_mailjet_sync" type="checkbox" id="activate_mailjet_sync" value="1" 
        <?php
    ...
    <div id="activate_mailjet_sync_form" class="
    <?php
    ...
    ">
        <div class="mailjet_sync_options_div">
            <div id="contact_list" class="mailjet_sync_woo_div">
                <div id="div-for-ajax" class="mj-woocommerce-contacts 
                <?php
                ...
                ">
					...
					?>
&nbsp&nbsp<a href="#" onclick="displaySyncListChoice()">
<?php
echo esc_attr($changeList);
?>
</a></span>
                </div>
                <div id="changeSyncList" 
                <?php
                echo (bool) esc_attr( ! $displaySyncListChoice) ? 'class="mj-hide"' : '';
                ?>
                >
                    <label for="mailjet_sync_list" class="mj-wp-sync-label">
                    <?php
...
> 
                    ...
                    <label id="checkbox-add-users" class="checkboxLabel">
                        ...
                        <span>
                        <?php
                        \printf(__('Also, add existing %s WordPress users (initial synchronization)', 'mailjet-for-wordpress'), $wpUsersCount);
                        ?>
                        </span>
                    </label>
                </div>
    ...
    <label class="mailjet-label" for="activate_mailjet_comment_authors_sync">
    <?php
    _e('Opt-in inside « Leave a reply » form', 'mailjet-for-wordpress');
    ?>
    </label>
    <label class="checkboxLabel">
        <input name="activate_mailjet_comment_authors_sync" type="checkbox" id="activate_mailjet_comment_authors_sync" value="1" 
        <?php
    ...
    <div id="comment_authors_contact_list" class="mailjet_sync_woo_div 
    <?php
    echo esc_attr($mailjetCommentAuthorsSyncActivated) == 1 ? 'mj-show' : 'mj-hide';
    ?>
    ">
        <div class="mailjet_sync_options_div">
            <label for="mailjet_comment_authors_list" class="mj-wp-sync-label">
            <?php
            _e('Mailjet contact lists', 'mailjet-for-wordpress');
            ?>
            </label>
...
<input name="settings_step" type="hidden" id="settings_step" value="subscription_options_step"><?php
