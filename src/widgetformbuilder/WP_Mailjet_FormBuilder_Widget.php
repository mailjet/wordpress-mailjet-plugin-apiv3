...
class WP_Mailjet_FormBuilder_Widget extends \WP_Widget
{
    ...
    public const WIDGETS_OPTIONS_NAME = 'widget_wp_mailjet_form_builder_widget';
    ...
    public function __construct()
    {
        ...
        parent::__construct($this->get_widget_slug(), __('Mailjet FormBuilder Widget', 'mailjet-for-wordpress'), $widget_options);
     ...
     * Send subscription email if you need
    ...
    public function sendSubscriptionEmail(): void
    {
        ...
        if (!(isset($instance[$locale], $instance[$locale]['language_checkbox']) && $instance[$locale]['language_checkbox'])) {
            ...
            $selectedLocales = array_filter($instance, static function ($localeInstance) {
        ...
        if (empty($_POST['subscription_email'])) {
            echo !empty($instance[$locale]['empty_email_message_input']) ? $instance[$locale]['empty_email_message_input'] : Mailjeti18n::getTranslationsFromFile($locale, 'Please provide an email address');
            wp_die();
        }
        ...
        if (!empty($properties) && is_array($this->mailjetContactProperties) && !empty($this->mailjetContactProperties)) {
            foreach ($properties as $propertyId => $propertyValue) {
                ...
                foreach ($this->mailjetContactProperties as $mailjetContactProperty) {
                    if ((int)$propertyId === (int)$mailjetContactProperty['ID']) {
        ...
        $sendingResult = $subscriptionOptionsSettings->mailjet_subscribe_confirmation_from_widget(
            $subscription_email,
            $instance,
            $subscription_locale,
            $widgetId
        ...
        if ($sendingResult) {
            echo !empty($instance[$locale]['confirmation_email_message_input']) ? $instance[$locale]['confirmation_email_message_input'] : Mailjeti18n::getTranslationsFromFile($locale, 'Subscription confirmation email sent. Please check your inbox and confirm your subscription.');
        } else {
            echo !empty($instance[$locale]['technical_error_message_input']) ? $instance[$locale]['technical_error_message_input'] : Mailjeti18n::getTranslationsFromFile($locale, 'A technical issue has prevented your subscription. Please try again later.');
        }
        wp_die();
    }
    ...
    private function activateConfirmSubscriptionUrl()
    {
        ...
        $params = array('subscription_email' => $subscription_email, 'subscription_locale' => $locale, 'list_id' => $list_id, 'thanks_id' => $thanks_id, 'properties' => $properties);
        if ($widget_id) {
        ...
        if (!$this->mailjetContactProperties) {
            $mailjetContactProperties = $this->getMailjetContactProperties();
        }
        if (!empty($this->mailjetContactProperties)) {
    ...
    public function update($new_instance, $old_instance)
    {
        ...
        if (version_compare($wp_version, '4.4', '<')) {
            ...
            foreach ($languages as $language => $locale) {
                // Do not save if language is active but there is no contact list chosen for it
                if (isset($new_instance[$locale]['language_checkbox'], $new_instance[$locale]['list']) && $new_instance[$locale]['list'] == "0"
                ) {
                    continue;
                }
                // Initial
    ...
    private function checkTransition($instance)
    {
        if (isset($instance['enableTaben']) && $instance['enableTaben'] == 'on') {
            $contactProperties0name = isset($instance['metaPropertyName1en']) ? $instance['metaPropertyName1en'] : false;
            // -> contactProperties0
            $contactProperties1name = isset($instance['metaPropertyName2en']) ? $instance['metaPropertyName2en'] : false;
            // -> contactProperties1
            $contactProperties2name = isset($instance['metaPropertyName3en']) ? $instance['metaPropertyName3en'] : false;
            // -> contactProperties2
    ...
    /**
     * @return SubscriptionOptionsSettings|null
     */
     ...
     * @return void
     */
    public function initPropertyData(): void
    {
        if (!$this->propertyData) {
            $this->mailjetContactProperties = $this->getMailjetContactProperties();
            if (!empty($this->mailjetContactProperties) && is_array($this->mailjetContactProperties)) {
                foreach ($this->mailjetContactProperties as $mjContactProperty) {
                    $this->propertyData[$mjContactProperty['ID']] = [
