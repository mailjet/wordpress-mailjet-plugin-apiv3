...
namespace MailjetWp\Bamarni\Composer\Bin\Command;

use MailjetWp\Bamarni\Composer\Bin\Config\Config;
use MailjetWp\Bamarni\Composer\Bin\Config\ConfigFactory;
use MailjetWp\Bamarni\Composer\Bin\ApplicationFactory\FreshInstanceApplicationFactory;
use MailjetWp\Bamarni\Composer\Bin\Input\BinInputFactory;
use MailjetWp\Bamarni\Composer\Bin\Logger;
use MailjetWp\Bamarni\Composer\Bin\ApplicationFactory\NamespaceApplicationFactory;
use MailjetWp\Composer\Command\BaseCommand;
use MailjetWp\Composer\Factory;
use MailjetWp\Composer\IO\IOInterface;
use MailjetWp\Composer\IO\NullIO;
use MailjetWp\Symfony\Component\Console\Application;
use MailjetWp\Symfony\Component\Console\Input\InputArgument;
...
class BinCommand extends BaseCommand
{
     ...
     */
    private $logger;
    public function __construct(?NamespaceApplicationFactory $applicationFactory = null, ?Logger $logger = null)
    {
    ...
    {
        $this->setDescription('Run a command inside a bin namespace')->addArgument(self::NAMESPACE_ARG, InputArgument::REQUIRED)->ignoreValidationErrors();
    }
    ...
    public function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        $this->logger->logDebug(sprintf('Current working directory: <comment>%s</comment>', $currentWorkingDir));
        ...
        $binInput = BinInputFactory::createInput($namespace, $input);
        return self::ALL_NAMESPACES !== $namespace ? $this->executeInNamespace($currentWorkingDir, $vendorRoot . '/' . $namespace, $binInput, $output) : $this->executeAllNamespaces($currentWorkingDir, $vendorRoot, $binInput, $output);
    }
    /**
     * @return list<string>
     */
    ...
    private function executeAllNamespaces(string $originalWorkingDir, string $binVendorRoot, InputInterface $input, OutputInterface $output) : int
    {
        ...
        foreach ($namespaces as $namespace) {
            $exitCode += $this->executeInNamespace($originalWorkingDir, $namespace, $input, $output);
        }
        return min($exitCode, self::FAILURE);
    }
    private function executeInNamespace(string $originalWorkingDir, string $namespace, InputInterface $input, OutputInterface $output) : int
    {
        $this->logger->logStandard(sprintf('Checking namespace <comment>%s</comment>', $namespace));
        try {
            self::createNamespaceDirIfDoesNotExist($namespace);
        } catch (CouldNotCreateNamespaceDir $exception) {
            $this->logger->logStandard(sprintf('<warning>%s</warning>', $exception->getMessage()));
            return self::FAILURE;
        }
        ...
        $namespaceApplication = $this->applicationFactory->create($this->getApplication());
        ...
        $this->logger->logDebug(sprintf('Running <info>`@composer %s`</info>.', $namespaceInput->__toString()));
        try {
    ...
    private static function createNamespaceDirIfDoesNotExist(string $namespace) : void
    {
        ...
        $mkdirResult = mkdir($namespace, 0777, \true);
    ...
    private function configureBinLinksDir(Config $config) : void
    {
        ...
        putenv(sprintf('COMPOSER_BIN_DIR=%s', $binDir));
        $this->logger->logDebug(sprintf('Configuring bin directory to <comment>%s</comment>.', $binDir));
    }
    private function ensureComposerFileExists() : void
    {
        ...
        $this->logger->logDebug(sprintf('Created the file <comment>%s</comment>.', $namespaceComposerFile));
    }
    ...
    {
        chdir($dir);
        $this->logger->logDebug(sprintf('Changed current directory to <comment>%s</comment>.', $dir));
    }
}
