...
use MailjetWp\GuzzleHttp\Promise as P;
use MailjetWp\GuzzleHttp\Promise\FulfilledPromise;
use MailjetWp\GuzzleHttp\Promise\PromiseInterface;
use MailjetWp\GuzzleHttp\Psr7\LazyOpenStream;
use MailjetWp\GuzzleHttp\TransferStats;
use MailjetWp\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \MailjetWp\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \MailjetWp\GuzzleHttp\Psr7\Utils::streamFor($sink);
