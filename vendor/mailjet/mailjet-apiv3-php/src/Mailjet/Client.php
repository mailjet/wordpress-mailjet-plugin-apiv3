...
class Client
{
    ...
    private $requestOptions = [self::TIMEOUT => 15, self::CONNECT_TIMEOUT => 2];
    private $smsResources = ['send', 'sms', 'sms-send'];
    private $dataAction = ['csverror/text:csv', 'csvdata/text:plain', 'JSONError/application:json/LAST'];
    /**
     ...
     */
    public function __construct(string $key, string $secret = null, bool $call = \true, array $settings = [])
    {
    ...
    {
        $args = \array_merge(['id' => '', 'actionid' => '', 'filters' => [], 'body' => 'GET' === $method ? null : '{}'], \array_change_key_case($args));
    ...
    {
        $path = 'REST';
        if (\in_array($resource, $this->smsResources, \true)) {
            $path = '';
        } elseif (\in_array($action, $this->dataAction, \true)) {
            $path = 'DATA';
