...
use MailjetWp\GuzzleHttp\Client as GuzzleClient;
use MailjetWp\GuzzleHttp\ClientTrait as GuzzleClientTrait;
use MailjetWp\GuzzleHttp\Exception\ClientException;
use MailjetWp\GuzzleHttp\Exception\ServerException;
use MailjetWp\GuzzleHttp\Exception\GuzzleException;
...
use MailjetWp\Psr\Http\Message\UriInterface;
class Request
{
     ...
     */
    public function __construct(array $auth, string $method, string $url, array $filters, $body, string $type, array $requestOptions = [])
    {
        $this->type = $type;
        $this->auth = $auth;
        $this->method = $method;
        ...
        $this->guzzleClient = new GuzzleClient(['defaults' => ['headers' => ['user-agent' => Config::USER_AGENT . \PHP_VERSION . '/' . Client::WRAPPER_VERSION]]]);
    }
    /**
    ...
    {
        $payload = ['query' => $this->filters, 'application/json' === $this->type ? 'json' : 'body' => $this->body];
        ...
        if ($call) {
            try {
                $response = \call_user_func_array([$this, \strtolower($this->method)], [$this->url, $payload]);
